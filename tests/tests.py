...
class TestShootShipUser(unittest.TestCase):
    def setUp(self):
        self.app = seawar_server.app.test_client()
    ...
    def test_set_user_shoot_miss(self):
        with patch('seawar_server.ShipService.shoot_to', return_value=False) as mock:
            resp = self.app.post(self.url, data=dict(x=2, y1=2))
        self.assertEqual(resp.status_code, 200)

        data = resp.json
        self.assertEqual(data.keys(), {'signal', 'x', 'y'})
        self.assertEqual(list(data.values()), ['miss', 2, 2])

    def test_set_user_shoot_hit(self):
        with patch('seawar_server.ShipService.shoot_to', return_value=True),\
